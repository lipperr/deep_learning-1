...
class CrossEntropyLoss(Criterion):
    ...
    def compute_output(self, input: np.ndarray, target: np.ndarray) -> float:
        ...
        lp = self.log_softmax.compute_output(input)
        return -1/input.shape[0] * np.sum(lp[np.arange(len(lp)), target])
    ...
    def compute_grad_input(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        ...
        grad[np.arange(len(grad)), target] = -1/input.shape[0]
        
        return self.log_softmax.compute_grad_input(input, grad)
